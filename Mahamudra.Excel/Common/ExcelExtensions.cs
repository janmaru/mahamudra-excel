    ...
    public static class ExcelExtensions
    {
        ...
        public static DataTable ReadExcel(this MemoryStream stream)
        {
            ...
            foreach (var cell in rows.ElementAt(0).Cast<Cell>()) 
                table.Columns.Add(spreadSheetDocument.GetCellValue(cell)); 

            foreach (var row in rows)
            {
                var tempRow = table.NewRow();
                for (var i = 0; i < row.Descendants<Cell>().Count(); i++) 
                    tempRow[i] = spreadSheetDocument.GetCellValue(row.Descendants<Cell>().ElementAt(i)); 
                table.Rows.Add(tempRow);
            }
            table.Rows.RemoveAt(0); //headers
            return table;
        }

        private static string GetCellValue(this SpreadsheetDocument document, Cell cell)
        {
